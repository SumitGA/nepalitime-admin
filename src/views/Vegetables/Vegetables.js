...
const VegetableRow = (props) => {
  ...
  return (
    <tr key={vegetable._id}>
      <th scope="row">
        <Link to={vegetableLink}>{vegetable._id}</Link>
      </th>
      <td>
        <Link to={vegetableLink}>{vegetable.name}</Link>
      </td>
      <td>{vegetable.min}</td>
      <td>{vegetable.max}</td>
      ...
      <td>
        <Link
          ...
          component={() => (
            <ImageUploader
              params={`${vegetable._id}`}
              updateVegetable={props.updateVegetable}
            />
          )}
          onLoad={(e) => this.uploadHandler()}
        >
          <Badge color={"primary"}>Upload Image</Badge>
        </Link>
      </td>
      <td>
        {vegetable.image_url !== "" ? (
        ...
        {vegetableList.length > 0 ? (
          <Row>
            <Col xl={12}>
              <Card>
                <CardHeader>
                  <i className="fa fa-align-justify"></i> Vegetables{" "}
                  ...
                  <Table responsive hover>
                    ...
                    <tbody>
                      ...
                      {vegetableList.map((vegetable, index) => (
                        <VegetableRow
                          key={index}
                          updateVegetable={this.getVegetablesList}
